...
angular.module('smartFeedbackApp')
    .controller('CompletareFeedbackCtrl', ['$scope', '$interval', '$cookies', 'FeedbackService', '$routeParams',
    function ($scope, $interval, $cookies, FeedbackService, $routeParams) {
      ...
      $scope.loadStat = function(){
        FeedbackService.loadStatistics($scope.feedbackId).then(function (response) {
          if (response !== "") {
            if (response.numarFeedbackuri === 0) {
              $scope.nrFeedbacks = 0;
            ...
            if ($scope.nrFeedbacks !== response.numarFeedbackuri) {
                ...
                data: [
                  {
                    ...
                    dataPoints: [
                      ...
                      {
                        y: response.organizareMedie,
                        label: response.organizareMedie.toString(),
                        indexLabel: "Organizare"
                      },
                      {
